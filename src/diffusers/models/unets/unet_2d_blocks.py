...
def get_down_block(
    ...
    elif down_block_type == "DownBlock2D_me":
        return DownBlock2D_me(
            num_layers=num_layers,
            in_channels=in_channels,
            out_channels=out_channels,
            temb_channels=temb_channels,
    ...
    elif down_block_type == "CrossAttnDownBlock2D_me":
        if cross_attention_dim is None:
            raise ValueError("cross_attention_dim must be specified for CrossAttnDownBlock2D")
        return CrossAttnDownBlock2D_me(
            num_layers=num_layers,
            transformer_layers_per_block=transformer_layers_per_block,
            in_channels=in_channels,
            out_channels=out_channels,
...
def get_up_block(
    ...
    elif up_block_type == "UpBlock2D_me":
        return UpBlock2D_me(
            num_layers=num_layers,
            in_channels=in_channels,
            out_channels=out_channels,
    ...
    elif up_block_type == "CrossAttnUpBlock2D_me":
        if cross_attention_dim is None:
            raise ValueError("cross_attention_dim must be specified for CrossAttnUpBlock2D")
        return CrossAttnUpBlock2D_me(
            num_layers=num_layers,
            transformer_layers_per_block=transformer_layers_per_block,
            in_channels=in_channels,
            out_channels=out_channels,
    ...
    elif up_block_type == "CrossAttnUpBlock2D_me_last":
        if cross_attention_dim is None:
            raise ValueError("cross_attention_dim must be specified for CrossAttnUpBlock2D")
        return CrossAttnUpBlock2D_me_last(
            num_layers=num_layers,
            transformer_layers_per_block=transformer_layers_per_block,
            in_channels=in_channels,
            out_channels=out_channels,
...
class CrossAttnDownBlock2D_me(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
class DownBlock2D_me(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
class CrossAttnUpBlock2D_me(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
    ...
    def forward(
        ...
        attention_mask: Optional[torch.FloatTensor] = None,
        encoder_attention_mask: Optional[torch.FloatTensor] = None,
        return_res_samples: Optional[bool]=False, idx: int = 0,
        up_block_add_samples: Optional[torch.FloatTensor] = None,
    ) -> torch.FloatTensor:
        lora_scale = cross_attention_kwargs.get("scale", 1.0) if cross_attention_kwargs is not None else 1.0
        is_freeu_enabled = (
        ...
        for i, (resnet, attn) in enumerate(zip(self.resnets, self.attentions)):
            ...
            if up_block_add_samples is not None:
                module = getattr(self, f"up_{i}")
                module_output = module(hidden_states, up_block_add_samples.pop(0))
                hidden_states = hidden_states + module_output
            else:
...
class CrossAttnUpBlock2D_me_last(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
    ...
    def forward(
        ...
        if self.upsamplers is not None:
            ...
            if up_block_add_samples is not None:
                ...
                module_output = module(hidden_states, up_block_add_samples.pop(0))
                hidden_states = hidden_states + module_output
                print('===========CrossAttnUpBlock2D_me_last_upsamplers')
        if return_res_samples:
            return hidden_states, output_states
        else:
            return hidden_states
    ...
    def forward(
        ...
        for resnet, attn in zip(self.resnets, self.attentions):
            ...
            if up_block_add_samples is not None:
                ...
                print(idx, 'up', hidden_states.shape)
